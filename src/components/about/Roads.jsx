...
const Roads = ({ currentLanguage }) => {
    ...
    const fetchData = async () => {
        try {
            let url = `${apiUrl}/api/roads-bridges-constructions?populate=*`;
    ...
    return (
        <div className='absolute top-0 lg:px-8 w-full h-full flex items-center lg:justify-end overflow-x-auto'>
            <div className="inline-block min-w-full shadow-sm rounded-lg">
                <p className='font-custom font-extrabold lg:text-3xl 2xl:text-4xl text-white rounded-md bg-[#313232] uppercase bg-opacity-75 p-3 2xl:py-10'>
                    {currentLanguage === "en"
                    ...
                    <tbody>
                        {currentProjects?.map(project => (
                            <tr key={project.id} className='font-custom1 text-white'>
                                <td className="text-center border-l-0 lg:text-left border lg:px-4 py-2 text-sm 2xl:text-lg">
                                    {currentLanguage === "en"
                                        ? project?.attributes?.projectName
                                        : project?.attributes?.localizations?.data[0]?.attributes?.projectName}
                                </td>
                                <td className="text-center lg:text-left border lg:px-4 py-2 text-sm 2xl:text-lg">
                                    {currentLanguage === "en"
                                ...
                                <td className="text-center lg:text-left border lg:px-4 py-2 text-sm 2xl:text-lg lg:text-base">
                                    {
                                        currentLanguage === "en"
                                            ...
                                            : project?.attributes?.localizations?.data[0]?.attributes?.projectStart + " - " + project?.attributes?.localizations?.data[0]?.attributes?.projectFinish
                                    }
                                </td>
                                <td className="text-center lg:text-left border-r-0 border lg:px-4 py-2 text-sm 2xl:text-lg">
                                    {currentLanguage === "en"
